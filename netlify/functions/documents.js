    ...
    switch (httpMethod) {
      ...
      case 'POST':
        ...
        if (authUser.role !== 'admin') {
          console.log(`[DOCUMENTS_SECURITY] ${new Date().toISOString()} - Forbidden: role '${authUser.role}' tried POST from ${clientIP}`);
          return {
            statusCode: 403,
            headers,
      ...
      case 'PUT':
        ...
        if (putAuthUser.role !== 'admin') {
          console.log(`[DOCUMENTS_SECURITY] ${new Date().toISOString()} - Forbidden: role '${putAuthUser.role}' tried PUT from ${clientIP}`);
          return {
            statusCode: 403,
            headers,
            body: JSON.stringify({ error: 'Forbidden' })
      ...
      case 'DELETE':
        ...
        if (deleteAuthUser.role !== 'admin') {
          console.log(`[DOCUMENTS_SECURITY] ${new Date().toISOString()} - Forbidden: role '${deleteAuthUser.role}' tried DELETE from ${clientIP}`);
          return {
            statusCode: 403,
            headers,
            body: JSON.stringify({ error: 'Forbidden' })
